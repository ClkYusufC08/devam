        ...
        try {
            ...
            while (cevap.length !== confirmation.length && point < 6) {
                await message.channel.send(stripIndents`
                    ${displayText === null ? '**Kartal Bot Adam Asmaca**!' : displayText ? '**Çok iyisin!**' : '**Yanlış Harf!**'}
                         **Kelime:**    \`${display.join(' ')}\`
            ...
            return message.channel.send(`Maalesef bilemedin kelime buydu: **${cevap}** Üzülme Bir Dahakine Artık`);
        } catch (err) {
            oyndurum.delete(message.channel.id);
